...
class CitationChecker(Middleware):
    ...
    def after_agent(self, ctx, report):
        ...
        for claim in claims:
            ...
            current_doc = ctx.corpus.get(claim["doc_id"])
            if current_doc and current_doc.body and any(claim_text in line for line in current_doc.body.splitlines()):
                continue

            for doc in ctx.corpus.docs:
                if doc.body and (doc.body in ctx.observed_text):
                    if any(claim_text in line for line in doc.body.splitlines()):
                        claim["doc_id"] = doc.doc_id
                        break

        report["citations"] = sorted(list(set(c["doc_id"] for c in claims if isinstance(c, dict) and c.get("doc_id"))))
        return report
